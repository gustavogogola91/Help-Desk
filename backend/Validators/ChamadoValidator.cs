...
namespace backend.Validators
{
    ...
    {
        // TODO: Adicionar EquipamentoRepository para validação de existencia do equipamento
        ...
        public ChamadoValidator(IEquipamentoRepository equipamentoRepository, ISetorRepository setorRepository, IEstabelecimentoRepository estabelecimentoRepository)
        {
            ...
            RuleFor(c => c.EquipamentoId)
                ...
                .MustAsync(async (setorId, cancellation) => await _equipamentoRepository.ExisteAsync(setorId)).WithMessage("O equipamento com o ID especificado não foi encontrado.");

            RuleFor(c => c.SetorSolicitanteId)
                .GreaterThan(0).WithMessage("O ID do setor deve ser maior que zero")
                .MustAsync(async (setorId, cancellation) => await _setoreRepository.ExisteAsync(setorId)).WithMessage("O Setor com o ID especificado para {PropertyName} não foi encontrado.");
            ...
            RuleFor(c => c.EstabelecimentoId)
                .GreaterThan(0).WithMessage("O ID do equipamento deve ser maior que zero")
                .MustAsync(async (estabelecimentoId, cancellation) => await _estabelecimentoRepository.ExisteAsync(estabelecimentoId)).WithMessage("O Estabelecimento com o ID especificado não foi encontrado.");
